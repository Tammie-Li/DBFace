...
class TitleBar(QWidget):
    ...
    def __init__(self, *args, title='', **kwargs):
        ...
        self.setupUi()
        # 支持设置背景
        self.setMaximumWidth(0xFFFFF)
        self.setAttribute(Qt.WA_StyledBackground, True)
        self.mPos = None
        # 找到父控件(或者自身)
        self._root = self.window()

        self.labelTitle.setFont(QFont("宋体", 17))
    ...
    def setupUi(self):
        ...
        self.setMaximumSize(0xFFFFF, self.Radius)
        ...
        layout.addItem(QSpacerItem(
            40, 20, QSizePolicy.Expanding, QSizePolicy.Minimum))
        # 最小化,最大化,还原,关闭按钮
        str = ['fa.window-minimize', 'fa.window-maximize', 'fa.window-restore', 'fa.times']
        for name, text in (('buttonMinimum', 0), ('buttonMaximum', 1),
                           ('buttonNormal', 2), ('buttonClose', 3)):
            button = QPushButton(self)
            button.setIcon(qta.icon(str[text], scale_factor= 1))
